...
def rebuild_dep(config, rehash_start=False):
    ...
    for order in orders:
        ...
        if receipt_number == 1:
            ...
            revenue_counter += order.sum_vat_normal * 100.0 + order.sum_vat_discounted_1 * 100.0 + order.sum_vat_discounted_2 * 100.0 + order.sum_vat_null * 100.0 + order.sum_vat_special * 100.0
